...
func buildManifest() manifest {
	return manifest{
		...
		Commands: []commandEntry{
			...
			{Command: "retask comment list", Description: "List comments on a task (all comments — top-level and replies — by default). Requires workspace context (--workspace-id or NWEB_WORKSPACE_ID). --task is the task ID (mapped to the task NRN). Pass --parent-comment-id to list only the replies under a specific comment. --created-by filters by author member NRN (repeatable, format: nweb:workspace:member:<uuid>). --sort: default, created-asc, created-desc", Flags: []string{"--task", "--parent-comment-id", "--sort", "--created-by"}, Example: "retask comment list --task <task-id>"},

...
func buildManifest() manifest {
	return manifest{
		...
		Commands: []commandEntry{
			...
			{Command: "retask auth pat list", Description: "List PATs for current user. --fields keeps only the listed output fields, in order (@short: pat_id, workspace_id, name)", Flags: []string{"--fields"}, Example: "retask auth pat list"},
			...
			{Command: "retask workspace list", Description: "List workspaces accessible to the user. --fields keeps only the listed output fields, in order (@short: workspace_id, name)", Flags: []string{"--fields"}, Example: "retask workspace list"},
			...
			{Command: "retask workspace member list", Description: "List workspace members. --fields keeps only the listed output fields, in order (@short: workspace_member_id, workspace_id, display_name)", Flags: []string{"--fields"}, Example: "retask workspace member list <workspace-id>"},
			...
			{Command: "retask customer list", Description: "List customers (admin only). --fields keeps only the listed output fields, in order (@short: customer_id, name)", Flags: []string{"--fields"}, Example: "retask customer list"},
			{Command: "retask customer get", Description: "Get a customer by ID", Example: "retask customer get <customer-id>"},
			{Command: "retask project list", Description: "List projects in workspace. --fields keeps only the listed output fields, in order (@short: project_id, workspace_id, key, name)", Flags: []string{"--archived", "--fields"}, Example: "retask project list"},
			...
			{Command: "retask project member list", Description: "List project members. --fields keeps only the listed output fields, in order (@short: project_member_id, project_id, member)", Flags: []string{"--fields"}, Example: "retask project member list <project-id>"},
			...
			{Command: "retask file list", Description: "List files in the workspace, optionally filtered by attached resource or author. --fields keeps only the listed output fields, in order (@short: file_id, workspace_id, file_name)", Flags: []string{"--target", "--created-by", "--fields"}, Example: "retask file list --target nweb:retask-task:task:<task-id>"},
			...
			{Command: "retask integration provider list", Description: "List integration providers. --fields keeps only the listed output fields, in order (@short: provider_id, name)", Flags: []string{"--fields"}, Example: "retask integration provider list"},
			...
			{Command: "retask integration list", Description: "List integrations. --fields keeps only the listed output fields, in order (@short: integration_id, workspace_id, provider_id)", Flags: []string{"--provider-id", "--fields"}, Example: "retask integration list --provider-id github"},
			...
			{Command: "retask integration gitlab repos", Description: "List GitLab repos accessible via integration", Flags: []string{"--level"}, Example: "retask integration gitlab repos"},
			{Command: "retask task list", Description: "List tasks. --assignee is repeatable and accepts workspace_member_nrns (format: nweb:workspace:member:<uuid>, obtainable from retask auth whoami or retask workspace member list). --fields keeps only the listed output fields, in order (@short: task_id, workspace_id, key, title)", Flags: []string{"--project-id", "--status", "--assignee", "--priority", "--fields"}, Example: "retask task list --assignee nweb:workspace:member:<uuid1> --assignee nweb:workspace:member:<uuid2>"},
			...
			{Command: "retask task create", Description: "Create a task. Requires workspace context (--workspace-id or NWEB_WORKSPACE_ID). --description accepts simple HTML. --status takes a status ID and --task-type a task type ID, both of which must exist in the project's config. --parent-task-id makes the task a subtask of that task. --assignee is repeatable and takes workspace_member_nrns (format: nweb:workspace:member:<uuid>); --reporter takes the same NRN format and defaults to the creator", Flags: []string{"--project-id", "--title", "--description", "--priority", "--status", "--task-type", "--due-at", "--parent-task-id", "--assignee", "--reporter", "--estimation-points"}, Example: "retask task create --project-id <id> --title 'Fix bug' --priority HIGH --status <status-id> --task-type <type-id>"},
			...
			{Command: "retask sandbox list", Description: "List sandboxes. --fields keeps only the listed output fields, in order (@short: sandbox_id, workspace_id, name)", Flags: []string{"--status", "--type", "--fields"}, Example: "retask sandbox list --type PRIVATE --fields @short,status"},
			{Command: "retask sandbox get", Description: "Get a sandbox by ID", Example: "retask sandbox get <sandbox-id>"},
			{Command: "retask sandbox create", Description: "Create a sandbox. --sharing sets who can use it (WORKSPACE_EDIT, WORKSPACE_VIEW, PRIVATE; default PRIVATE) — agents can only run on a WORKSPACE_* sandbox. Custom config via --env/--git-repo/--startup-command/--session-init-command/--shutdown-policy/--integration-provider-id, all mutually exclusive with --template-id", Flags: []string{"--name", "--type", "--sharing", "--template-id", "--env", "--git-repo", "--startup-command", "--session-init-command", "--shutdown-policy", "--integration-provider-id"}, Example: "retask sandbox create --name my-sandbox --session-init-command 'claude --dangerously-skip-permissions \"$SEED_PROMPT\"'"},
			{Command: "retask sandbox update", Description: "Update a sandbox. --sharing changes who can use it: WORKSPACE_EDIT, WORKSPACE_VIEW, PRIVATE", Flags: []string{"--name", "--sharing"}, Example: "retask sandbox update <id> --sharing WORKSPACE_EDIT"},
			...
			{Command: "retask sandbox session list", Description: "List sessions. --fields keeps only the listed output fields, in order (@short: session_id, workspace_id, name)", Flags: []string{"--sandbox-id", "--status", "--fields"}, Example: "retask sandbox session list --sandbox-id <sb-id>"},
			...
			{Command: "retask agent list", Description: "List agents. --fields keeps only the listed output fields, in order (@short: agent_id, workspace_id, name)", Flags: []string{"--role", "--fields"}, Example: "retask agent list --role ROLE_TASK_PROCESSOR"},

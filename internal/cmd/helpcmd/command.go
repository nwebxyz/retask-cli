...
func buildManifest() manifest {
	return manifest{
		...
		Commands: []commandEntry{
			...
			{Command: "retask sandbox connect", Description: "Connect this machine as a Private VM sandbox (long-running). Session folders are created in the current directory and recorded in <sandbox-id>.json. Stopping a session, the sandbox, or this command leaves folders on disk; --retention deletes those older than its window (checked hourly), and \"off\" disables it. Live sessions are never deleted", Flags: []string{"--mode", "--auto-open", "--no-auto-respond", "--retention"}, Example: "retask sandbox connect <sandbox-id> --retention 30d"},
